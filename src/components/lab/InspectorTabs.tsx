...
import { Wrench, FileText, ClipboardCheck, Network } from "lucide-react";
import { cn } from "@/lib/utils";

export function InspectorTabs({ view }: { view: LabView }) {
  return (
...
function SystemPromptView({ view }: { view: LabView }) {
  const tokens = Math.round(view.systemPrompt.length / 4);
  return (
    <div className="space-y-3">
      ...
      <pre className="whitespace-pre-wrap rounded-lg border border-slate-200 bg-white p-3.5 font-mono text-[12px] leading-relaxed text-slate-800">
        {view.systemPrompt}
      </pre>
    </div>
  );
